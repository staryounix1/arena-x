...
function ArenaProvider({ children }: { children: ReactNode }) {
  ...
  const logout = () => { if (user) recordActivity('تسجيل الخروج', `غادر اللاعب ${user.username} المنصة`, 'ACCOUNT', user); if (supabaseEnabled && supabase) void supabase.auth.signOut(); setUser(null); };
   ...
   const requestWithdrawal = async (amount: number, method: string, destination: string, notes: string) => {
     ...
     const setMatchRoom = async (id: string, roomCode: string) => { if (!user || !roomCode.trim()) return false; if (supabaseEnabled && supabase) { const result = await supabase.rpc('set_match_room', { match_id_value: id, room_code_value: roomCode.trim() }); if (result.error) return false; } setMatches(old => old.map(item => item.id === id ? { ...item, room_code: roomCode.trim(), room_ready_at: new Date().toISOString(), room_setup_deadline_at: item.room_setup_deadline_at || new Date(Date.now() + 60 * 1000).toISOString(), match_deadline_at: undefined } : item)); return true; };
