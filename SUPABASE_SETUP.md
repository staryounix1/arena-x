# Supabase Setup Guide for ARENA//X
...
3. Run every file in `supabase/migrations/` in chronological filename order.
4. The wallet-critical files are `20260917_platform_hardening.sql` and `20260917_platform_hardening_fix.sql`; apply them after the earlier files and before the later ones. They remove direct wallet writes, protect verification fields, and make the wallet ledger immutable.

Before applying the wallet-hardening files, reconcile any negative balances and legacy open matches whose escrow status is not `HELD`.
